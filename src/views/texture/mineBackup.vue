...
export default defineComponent({
  ...
  setup () {
    ...
    let gui
    // 用于 gui 调试
    let pointLight, metal
    const excludes = ['科技线条', '平面2', '管道', '中间件_1', '2_2', '立方体3', '立方体', '立方体7', '立方体7_01', '立方体1_1', '立方体7_0', '扫描1_2', '扫描_1', '立方体2_1', '扫描1_3']
    ...
    const slotColor = ['集装箱1', '水塔', '立方体2_1', '立方体1', '立方体1_1', '立方体_1', '立方体_2']
    const towers = ['1_1', '2']
    ...
    function init () {
      ...
      const shelf = new THREE.MeshBasicMaterial({ color: 0x444539 })
      const carHandle = new THREE.MeshBasicMaterial({ color: 0xd7d5c1 })
      const carThr = new THREE.MeshBasicMaterial({ color: 0x2d2e1f })
      const carShanye = new THREE.MeshBasicMaterial({ color: 0x6a6948 })
      const carSlot = new THREE.MeshBasicMaterial({ color: 0x696d57 })
      const carTop = new THREE.MeshBasicMaterial({ color: 0x514f3c })
      // 河流
      // const track = new THREE.MeshBasicMaterial({ color: 0x84d6e9 })
      const boxLine = new THREE.MeshBasicMaterial({ color: 0xfdfdc5 })
      // 围栏
      ...
      // 使用着色器增加磨砂效果
      // 塔
      const tower = new THREE.MeshBasicMaterial({ color: 0x242c25 })
      // 架子
      const frames = new THREE.MeshBasicMaterial({ color: 0x969786 })
      // 管道
      const pipeline = new THREE.MeshBasicMaterial({ color: 0x716c5a })
      // 窗口
      const windowColor = new THREE.MeshBasicMaterial({ color: 0xe6e6d9 })
      // 屋顶盒子
      const roofBox = new THREE.MeshBasicMaterial({ color: 0x161814 })
      // 屋顶
      const roof = new THREE.MeshBasicMaterial({ color: 0xc9c7b4 })
      // 机械车
      const vehicle = new THREE.MeshBasicMaterial({ color: 0x313227 })
      // 地板
      ...
      const customMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          b: { type: 'f', value: 1.0 },
          p: { type: 'f', value: 2.0 },
          glowColor: { type: 'c', value: new THREE.Color(0x16a1f5) }
        },
      ...
      loader.load('models/mine.gltf', gltf => {
        ...
        model.rotation.set(Math.PI * 0.1, Math.PI * 0.25, 0)
        scene.add(model)
      })
    ...
    function buildGui () {
      ...
      const params = {
        ...
        'metal color': metal.color.getHex(),
      ...
      gui.addColor(params, 'metal color').onChange((val) => {
        metal.color.setHex(val)
        render()
      })

      gui.add(params, 'metal roughness', 0, 1).onChange((val) => {
        metal.roughness = val
        render()
      })
